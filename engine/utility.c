...
#if 0
void fvdi_qsort(void *base, long nmemb, long size, int (*compar) (const void *, const void *))
{
		...
		while (link)
		{
			if (((long) link & ADDR_NOT_OK) ||
				((unsigned int) (link->size & 0xffff) >=
				 sizeof(block_space) / sizeof(block_space[0])) || !(link->size >> 16))
				break;
