...
static void clc_arc(Virtual * vwk, long gdp_code, long xc, long yc, long xrad, long yrad,
			 ...
			 Fgbg fill_colour, Fgbg border_colour, short *pattern, short *points, long mode, long interior_style)
{
	...
	if (vwk->clip.on)
	{
		if (((xc + xrad) < vwk->clip.rectangle.x1)
			...
			|| ((yc + yrad) < vwk->clip.rectangle.y1) || ((yc - yrad) > vwk->clip.rectangle.y2))
			return;
	}
	start = angle = beg_ang;
