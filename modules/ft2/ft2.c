...
Fontheader *ft2_load_font(Virtual * vwk, const char *filename)
{
	...
	if (font)
	{
		...
		font->flags = FONTF_EXTERNAL |			/* FT2 handled font */
			(FT_IS_SCALABLE(face) ? FONTF_SCALABLE : 0) | (FT_IS_FIXED_WIDTH(face) ? FONTF_MONOSPACED : 0);	/* .FNT compatible flag */
...
static FT_Error ft2_load_glyph(Virtual * vwk, Fontheader * font, short ch, c_glyph * cached, int want)
{
	...
	if (!cached->index)
	{
		...
		{
			access->funcs.puts("FT2 MAPPING THAT SHOULD HAVE NEVER HAPPENED!!!\n");
			cached->index = ch;
		}
	}
...

		{
			...
			for (row = 0; row < current->rows; ++row)
			{
				...
				dst = (unsigned char *) textbuf->address +
					(row + glyph->yoffset) * textbuf->wdwidth * 2 + (offset >> 3);
