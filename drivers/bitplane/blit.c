...
static void do_blit(struct blit *blt)
{
	...
	kprintf("ENDMASK 0x%04x-%04x-%04x\n", (UWORD) end_1, (UWORD) blt->end_2, (UWORD) blt->end_3);
...
#define mSkewNFSR    0x40
	...
	s_addr = (ULONG) info->s_form +
		(ULONG) info->s_ymin * (ULONG) info->s_nxln + (ULONG) s_xmin_off *(ULONG) info->s_nxwd;
	d_addr = (ULONG) info->d_form +
		(ULONG) info->d_ymin * (ULONG) info->d_nxln + (ULONG) d_xmin_off *(ULONG) info->d_nxwd;
	...
	if (s_addr < d_addr)
	{
		...
		s_addr = (ULONG) info->s_form +
			(ULONG) info->s_ymax * (ULONG) info->s_nxln + (ULONG) s_xmax_off *(ULONG) info->s_nxwd;
		d_addr = (ULONG) info->d_form +
			(ULONG) info->d_ymax * (ULONG) info->d_nxln + (ULONG) d_xmax_off *(ULONG) info->d_nxwd;
...
long CDECL c_mouse_draw(Workstation * wk, long x, long y, Mouse * mouse)
{
	...
	{0, c_mouse_draw_1, c_mouse_draw_2, 0, c_mouse_draw_4, 0, 0, 0, c_mouse_draw_8};
