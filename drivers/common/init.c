...
long CDECL init(Access *_access, Driver *driver, Virtual *vwk, char *opts)
{
	...
	if ((default_wk = (Workstation *) access->funcs.malloc(sizeof(Workstation) + wk_extend, 3)) == 0)
		return 0;

	if ((default_vwk = (Virtual *) access->funcs.malloc(sizeof(Virtual), 3)) == 0)
	{
	...
	if (graphics_mode->flags & CHECK_PREVIOUS)
	{
		...
		if ((default_palette = (Colour *) access->funcs.malloc(wk->screen.palette.size * sizeof(Colour), 3)) == 0)
		{
